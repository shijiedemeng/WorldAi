...
@Setter
@Entity
@Table(name = "requirement_module_info")
public class RequirementModuleInfo extends BaseEntity {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    @Column(name = "module_no", nullable = false, unique = true, length = 64)
    ...
    @Column(name = "parent_requirement_no", nullable = false, length = 64)
    ...
    @Column(name = "mcp_file_search_enabled_flag", nullable = false)
    private Boolean mcpFileSearchEnabledFlag;

    @Column(name = "document_ids", length = 1024)
    private String documentIds;

    @Column(name = "project_knowledge_search_enabled_flag", nullable = false)
    private Boolean projectKnowledgeSearchEnabledFlag;

    @Column(name = "project_knowledge_search_limit")
    private Integer projectKnowledgeSearchLimit;

    @Column(name = "project_knowledge_search_min_score")
